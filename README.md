...
| 7 | `geoBlockToken` | Adds `geo_block_token` as a query parameter on every client HTTP request; see `examples/geoToken.ts`. |
| 8 | `useServerTime` | Uses `getServerTime()` when building authenticated headers instead of the local clock. |
